...
| 段階 | 内容 |
| --- | --- |
| 除外 | 媒体側でPR・広告と表記された記事、成人向け、求人などは掲載しない |
...
| 作品名の表記ゆれ | 空白・記号・【】〈〉の違いを無視し、`filters.json` の `works.aliases` で略称にそろえて（Re:ゼロ→リゼロ、転生したらスライムだった件→転スラ など）同じ作品とみなす。表示は一番多い表記。ゲーム版と同じ仕組み |
...
- 見たくない話題（事件・トラブル／熱愛・結婚／訃報）を話題ごとに非表示（`filters.json` で `sensitive: true` のカテゴリ。見出しだけで判定し、『』「」の中は無視）
...
- **予定**（`/calendar/`）: 見出しの「日付＋動詞」（放送・配信・公開・発売・開催）を拾う（`scripts/lib/filter.mjs` の `extractSchedules`、設定は `filters.json` の `schedule`）。
  年の無い日付は記事の日付から補い（150日以上前なら翌年）、「〜まで」の締め切り・「PV公開」・過去形・中止/延期は拾わない。同じ作品・同じ日・同じ動詞は1件にまとめる。
- **作品ページ**（`/work/<名前>/`）: 直近30日の記事が2件以上ある作品ごとに静的ページを作る。一覧は `/work/`。サイトマップにも載る。
- **RSS**: 全記事（`/feed.xml`）とカテゴリ別（`/feed/<カテゴリ>.xml`）。どれも日本語の記事の新しい50件。
