...
//
//  - URLに載せる(共有できる)条件: キーワード / カテゴリ / 期間 / 並び順 / 作品 / 言語
...
export async function startFeed() {
  ...
  function passes(it: Item, skip: "cats" | "sources" | null = null, query = parseQuery(state.q)) {
  ...
  function renderItem(it: Item): string {
    ...
    return `<article class="${classes}">
      <div class="item-grid">
        ...
        ${works || cats || extra ? `<div class="item-tags">${works}${extra}${cats ? `<span class="item-cats">${cats}</span>` : ""}</div>` : ""}
      </div>
    </article>`;
  }
  ...
  function renderControls() {
    ...
    if (state.onlyFollow) active.push(`<button type="button" class="chip" data-clear="onlyFollow">フォロー作品のみ</button>`);
  ...
  // 検索窓の下の候補(作品名・話題)。候補を選ぶと、キーワードではなくその絞り込みを入れる
  ...
  document.addEventListener("click", (e) => {
    ...
    const btn = el.closest<HTMLElement>("button, a[data-work-link]");
    if (!btn) return;
    const d = btn.dataset;
    if (d.workLink) {
      e.preventDefault();
