...
//
// アニメニュースの「収集時フィルタ」本体。fetch-news.mjs から使うほか、
// filter.test.mjs で単体テストしている。副作用のない純粋関数だけを置く。
//
// フィルタは次の順に効く(src/data/filters.json で調整できる):
//   1. hardExclude  … 広告・成人向け・求人などを無条件で捨てる
//   2. relevance    … フィードの種類(kind)ごとのしきい値で「アニメ関連か」を判定
//                     (アニメ専門媒体はしきい値0=全部採用、総合媒体ほど厳しい)
//   3. categories   … 新作・放送・PV…のカテゴリを複数付与(表示側の絞り込み用)
...
/**
 * アニメ関連度スコア。重み付きキーワードの合計 + 既知作品名ボーナス。
 ...
 *  - works.aliases で略称にそろえる(「Re:ゼロから始める異世界生活」→「リゼロ」など)
 *  - numeralPrefixes に挙げた略称の直後のローマ数字を数字にする(アニメ版は今のところ無し)
 * 「リゼロ」と「Re:ゼロから始める異世界生活」、「まどマギ〈廻天〉」と「魔法少女まどか☆マギカ〈ワルプルギスの廻天〉」は同じキーになる。
 */
 ...
 *  - 「」は発言の引用にも使われるので、直前が「アニメ」「劇場版」等のときだけ採用。
 *    lenient=true(アニメ専門媒体)なら、句読点や感嘆符を含まない短い「」も採用する
...
export function extractWorks(title, config, { lenient = false } = {}) {
  ...
  for (const m of text.matchAll(/「([^」]{1,40})」/g)) {
    ...
    if (ctx.test(before) || (lenient && looksLikeTitle)) found.push(inner);
  }
