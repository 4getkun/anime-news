...
//
// アニメニュース自動収集スクリプト。GitHub Actions から定期実行される。
//
...
const parser = new Parser({
  ...
  headers: {
    "User-Agent": "Mozilla/5.0 (compatible; AnimeNewsBot/1.0; +https://github.com/4getkun/anime-news)",
  },
  customFields: {
    item: [
...
async function main() {
  console.log(`アニメニュース収集を開始 (${FEEDS.length}フィード)`);
  ...
  // ---- 3. フィルタ ----
  // 作品辞書: アニメ専門媒体(日本語)の見出しから抽出した作品名。総合媒体の記事に
  // この作品名が出てくれば、「アニメ」という語が無くても関連記事として拾える。
